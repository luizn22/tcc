...
\begin{itemize}
    ...
    \item Desenvolvimento Iterativo: Melhoria contínua do software por meio de ciclos repetidos de desenvolvimento e teste;
...
Isso foi realizado tendo como base a uma sequência de ações prováveis que um usuário faria durante o uso da ferramenta.
...
\begin{table}[H]
    \begin{center}
        \begin{tabularx}{\textwidth}{|>{\bfseries\raggedright\arraybackslash\center}m{5cm}|X|}
            ...
            Objetivo                                        & Fornecer planilha de leiaute para serem inseridos os dados necessários para um determinado processo de identificação.                                                                                                                                                                 \\ \hline
            Atores                                          & Usuário                                                                                                                                                                                                                                                                                  \\ \hline
            Restrições                                      & Deve-se salvar a planilha referente ao leiaute necessário.                                                                                                                                                                                                                                \\ \hline
            Pré-condições                                   & -                                                                                                                                                                                                                                                                                        \\ \hline
            Cenário principal                               & 1. O método é chamado, recebendo o caminho onde deve salvar o arquivo e possivelmente os parâmetros referentes ao leiaute em específico, como tipo de arquivo, por exemplo.\newline 2. É construido em memória o layout desejado.\newline 3. É salvo o arquivo no caminho especificado. \\ \hline
...
\begin{table}[H]
    \begin{center}
        \begin{tabularx}{\textwidth}{|>{\bfseries\raggedright\arraybackslash\center}m{5cm}|X|}
            ...
            Cenário principal                               & 1. O método de identificação é chamado, recebendo o caminho do arquivo de leiaute e possivelmente os parâmetros referentes ao método em específico.\newline 2. São realizados os cálculos e é obtido o objeto de modelo do sistema.\newline 3. O objeto de modelo do sistema é retornado ao usuário. \\ \hline
...
\begin{table}[H]
    \begin{center}
        \begin{tabularx}{\textwidth}{|>{\bfseries\raggedright\arraybackslash\center}m{5cm}|X|}
            ...
            Atores                                          & Usuário                                                                                                                                                                                                                                                                                  \\ \hline
            Restrições                                      & Deve ser possível visualizar gráfico e dados indicadores importantes do modelo.                                                                                                                                                                                                          \\ \hline
            Pré-condições                                   & O Usuário possui um objeto de modelo do sistema.                                                                                                                                                                                                                                         \\ \hline
            Cenário principal                               & 1. Um método de visualização de dados ou gráfico é chamado, passando possíveis parâmetros necessários ou que irão mudar como os dados serão exibidos.\newline 2. São preparados os objetos a serem exibidos ao usuário.\newline 3. São exibidos os dados ou gráficos ao usuário. \\ \hline
            Pós-condições                                   & O Usuário pode visualizar os dados do modelo.                                                                                                                                                                                                                                            \\ \hline
        \end{tabularx}
...
\begin{table}[H]
    \begin{center}
        \begin{tabularx}{\textwidth}{|>{\bfseries\raggedright\arraybackslash\center}m{5cm}|X|}
            ...
            Pré-condições                                   & O Usuário possui um objeto de modelo do sistema.                                                                                                                                                                                                                                           \\ \hline
            Cenário principal                               & 1. O método de aproximação é chamado, recebendo o modelo do sistema e possivelmente os parâmetros referentes ao método de aproximação em específico.\newline 2. São realizados os cálculos e é obtido o objeto de controlador.\newline 3. O objeto de controlador é retornado ao usuário. \\ \hline
...
\begin{table}[H]
    \begin{center}
        \begin{tabularx}{\textwidth}{|>{\bfseries\raggedright\arraybackslash\center}m{5cm}|X|}
            ...
            Atores                                          & Usuário                                                                                                                                                                                                                                                                                  \\ \hline
            Restrições                                      & Deve ser possível visualizar gráfico e dados indicadores importantes do controlador gerado.                                                                                                                                                                                              \\ \hline
...
\begin{figure}[H]
    ...
    \fbox{\includegraphics[scale=0.32]{figuras/class_diag}}
...
\begin{figure}[H]
    ...
    \fbox{\includegraphics[scale=0.6]{figuras/class_diag_diubmi}}
...
Os métodos dessa classe foram criados conforme as necessidades encontradas durante o desenvolvimento, de forma que
...
\begin{figure}[H]
    ...
    \fbox{\includegraphics[scale=0.6]{figuras/class_diag_dupu}}
...
transferência.
De forma similar a classe DataUtils, também foram criados outros métodos para esta classe conforme as necessidades de
desenvolvimento.
...
possibilita o acesso às funcionalidades de visualização de dados a partir do objeto de modelo.
...
\begin{figure}[H]
    ...
    \fbox{\includegraphics[scale=0.7]{figuras/class_diag_model}}
...
referentes a um modelo, e não necessariamente apresentá-los ao usuário, e a classe PlotUtils, que fornece ferramentas
...
resposta do modelo a sinal degrau e comparação do modelo com os dados discretos utilizados para obtê-lo,
get\_model\_data, que retorna dados de resposta a sinal degrau do modelo, como sobressinal e tempo de acomodação, e
...
\begin{figure}[H]
    ...
    \fbox{\includegraphics[scale=0.7]{figuras/class_diag_controller}}
...
ControllerView implementa métodos idênticos a ModelView, mas com o foco no objeto controller, e com a diferença de que
...
Assim complementado as necessidades levantadas pelo caso de uso da tabela \ref{tab:uc4}.

\begin{figure}[H]
    \centering
    \caption{Diagrama de classes ampliado - BaseControllerAproximation}
    \fbox{\includegraphics[scale=0.6]{figuras/class_diag_bcacontroller}}
...
Ele é um sistema de controle de versões preferido da comunidade, devido ao seu alto desempenho, natureza decentralizada
e funcionalidades robustas para manipulação de grandes projetos \cite{usegit}.
Além de ser gratuito e de código aberto, é amplamente utilizado e integrado em uma pletora de ferramentas e repositórios
como GitHub.
...
adequação ao PEP 8, de forma que o resultado passa nas verificações de ambas as ferramentas.
...
De forma que ao rodá-lo, com base no arquivo de configuração criado, são instaladas as dependências do projeto em um
...
\begin{alineas}
    \item \textbf{autodoc}: Utilizada para gerar documentação automaticamente baseada nas \textit{docstrings};
...
Visando disponibilizar o código-fonte como código aberto e acessível, foi criado um repositório público e gratuito na plataforma GitHub, cujo link pode ser encontrado no apêndice \ref{ch:actgithub}, onde qualquer um pode ter acesso a todo
...
Este processo tornou a biblioteca acessível para instalação via pip para qualquer pessoa.
...
Tipicamente, o modelo matemático de uma planta no domínio da frequência pode ser definido por um numerador e um
...
(control), representando o modelo matemático de uma planta no domínio da frequência, além de guardar alguns outros
...
Classe utilizada para visualização de dados de um objeto da classe Model.
...
\subsubsection{BaseModelIdentification}\label{subsubsec:devBaseModelIdentification}
...
Como uma subclasse de BaseModelIdentification, implementa o método de identificação de Ziegler e Nichols,
descrito em \ref{subsubsec:znfun}.
Ela sobrescreve o método abstrato da classe pai get\_model, implementando para que, com base nos dados de resposta a
...
\begin{alineas}
    ...
    resposta do modelo;
    \item Então obtém a pandas.Series tf\_data (lista cujo índice representa o tempo e os valores a saída)
...
Com a única diferença sendo a determinação de $t2$ baseado em $y_f*0,632$ e o cálculo de $\theta$ baseado em $t_1$ e
$t_2$.
...
\begin{alineas}
    ...
    \item Então obtém a pandas.Series tf\_data (lista cujo índice representa o tempo e os valores a saída) e o valor do sinal degrau através do método DataUtils.setup\_data\_default;
...
Classes de controlador, representativas de um sistema em Malha Fechada, ocorrendo o controle de um Modelo por meio de um
Controlador PID\@.
...
Para isso, atua em cima do objeto de Função de Transferência da biblioteca de controle Model.tf
...
Classe utilizada para visualização de dados de um objeto da classe Controller.
...
\begin{alineas}
    ...
    de regime.
    Também realiza a plotagem da resposta a sinal degrau de Model, para comparação por padrão;
    \item \textbf{get\_model\_step\_response\_data}: Utiliza control.step\_info da biblioteca de controle para
    obtenção dos dados de resposta a sinal degrau do sistema no formato de dicionário do Python;
    \item \textbf{print\_model\_step\_response\_data}: Imprime em tela os dados de resposta a sinal degrau do sistema;
    \item \textbf{print\_tf}: Imprime em tela a função de transferência do modelo, com formatação matemática caso
...
controlador PID, que variam conforme o tipo de controlador e fazem uso exclusivo dos parâmetros $K$, $\tau$ e
$\theta$.
...
\begin{alineas}
    ...
    A intensidade da suavização depende do parâmetro smoothness;
    ...
    Ele faz o preparo dos dados de adicionando tempo de aquisição e sinal degrau, caso necessário,
    e aplicando um filtro linear, caso solicitado.
    Retorna a pandas.Series referente a série temporal do sinal de saída em relação ao tempo e o valor do sinal degrau;
